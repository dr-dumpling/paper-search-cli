...
export class SpringerSearcher extends PaperSource {
    ...
    async search(query, options = {}) {
        ...
        try {
            ...
            let response;
            if (useOpenAccess) {
                response = await ErrorHandler.retryWithBackoff(() => this.openAccessClient.get('/json', { params }), { context: 'Springer OpenAccess search' });
            }
            else {
                response = await ErrorHandler.retryWithBackoff(() => this.metadataClient.get('/json', { params }), { context: 'Springer Meta search' });
            }
    ...
    async downloadPdf(doi, options = {}) {
        ...
        try {
            const response = await ErrorHandler.retryWithBackoff(() => axios.get(paper.pdfUrl, { responseType: 'stream' }), { context: 'Springer download' });
    ...
    async getCitations(doi) {
        try {
            ...
            return await withTimeout(crossref.getCitations(doiValidation.sanitized), TIMEOUTS.HEALTH_CHECK, 'Crossref citation request timed out');
        }
        catch (error) {
    ...
    async getReferences(doi) {
        try {
            ...
            const crossref = new CrossrefSearcher();
            // Add timeout wrapper for Crossref API calls
            return await withTimeout(crossref.getReferences(doiValidation.sanitized), TIMEOUTS.HEALTH_CHECK, 'Crossref references request timed out');
        }
        catch (error) {
            // Don't log the DOI in case it's sensitive
     ...
     * 清理和转义查询参数中的特殊字符
     */
    sanitizeQueryValue(value, context = 'general') {
