...
export class ScopusSearcher extends PaperSource {
    ...
    async getAbstract(scopusId) {
        if (!this.apiKey) {
            throw new Error('Scopus API key is required');
        }
        try {
            await this.rateLimiter.waitForPermission();
            const response = await ErrorHandler.retryWithBackoff(() => this.client.get(`/content/abstract/scopus_id/${scopusId}`, {
                params: { view: 'FULL' }
            ...
            if (authorsData && authorsData.author) {
                ...
                authors = coredata['dc:creator'].map((c) => c.$).join(', ');
            }
     ...
     * 获取参考文献的Scopus ID列表
     */
    async getReferenceIds(scopusId) {
        if (!this.elsevierApiKey)
            return [];
        try {
            await this.rateLimiter.waitForPermission();
            const response = await ErrorHandler.retryWithBackoff(() => axios.get(`https://api.elsevier.com/content/abstract/scopus_id/${scopusId}`, {
                params: { view: 'REF' },
                headers: {
                    'Accept': 'application/json',
     ...
     * 获取引用文献的Scopus ID列表
     */
    async getCitationIds(scopusId) {
        if (!this.elsevierApiKey)
            return [];
        try {
            await this.rateLimiter.waitForPermission();
            const response = await ErrorHandler.retryWithBackoff(() => axios.get('https://api.elsevier.com/content/abstract/citations', {
     ...
     */
    async getPaperWithCitations(paperId) {
        try {
            const paper = await this.getAbstract(paperId);
            if (!paper)
                return null;
...
//# sourceMappingURL=ScopusSearcher.js.map