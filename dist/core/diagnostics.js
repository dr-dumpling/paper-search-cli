...
export function diagnoseToolResult(context) {
...
export function diagnosticContextFromCli(command, positionals, flags) {
    if (command === 'search') {
        return {
            tool: 'search_papers',
...
function diagnosePartialFailure(context, errorEntries) {
    const platforms = errorEntries.map(([source]) => source);
    const actions = new Set(['Inspect the `errors` object in the output for provider-specific messages.']);
...
function zeroResultDiagnostic(context) {
...
function platformFromTool(tool) {
    if (!tool)
        return undefined;
...
function requirementForPlatform(platform) {
    if (!platform)
        return undefined;
    return API_REQUIREMENTS.find(requirement => requirement.platform === platform);
}
function isMultiSourceResult(data) {
    return Boolean(data &&
        typeof data === 'object' &&
        !Array.isArray(data) &&
        'total' in data &&
...
function isKeyGroupConfigured(group) {
    if (group.length === 0)
        return true;
    return group.some(key => Boolean(process.env[key]));
}
function missingKeyActions(requirement) {
    if (!requirement)
        return [];
