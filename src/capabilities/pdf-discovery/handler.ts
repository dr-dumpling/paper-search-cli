...
export async function handleDownloadPaper(
  args: { paperId: string; platform: string; savePath?: string },
  searchers: Searchers
) {
...
export async function handleDownloadWithFallback(args: Parameters<typeof downloadWithFallback>[1], searchers: Searchers) {
