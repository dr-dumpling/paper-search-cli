...
function prepareOutput(result: unknown): { payload: unknown; exitCode?: number } {
  ...
  const { exitCode: _exitCode, ...payload } = result as Record<string, unknown>;
...
async function handleSetupCommand(positionals: string[], flags: Record<string, FlagValue>): Promise<unknown> {
  ...
  try {
    ...
    let selectedSkillTargets: string[] = [];
    if (!skipSkills) {
...
async function handleDoctorCommand(flags: Record<string, FlagValue>): Promise<unknown> {
...
async function handleSmokeCommand(flags: Record<string, FlagValue>): Promise<unknown> {
  ...
  if (flags.live === true || mode === 'live') {
    ...
    return runLiveSmoke({
      ...
      callTool: (tool, args) => callTool(tool, args, {}) as Promise<{ ok: boolean; message?: string; data?: unknown }>
    });
  }
...
function handleSkillsCommand(positionals: string[], flags: Record<string, FlagValue>): unknown {
...
async function promptSkillTargets(session: PromptSession, skillsRoot?: string): Promise<string[]> {
