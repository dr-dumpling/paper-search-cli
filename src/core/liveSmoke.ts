...
export interface LiveSmokeRunner {
  callTool(tool: string, args: Record<string, unknown>): Promise<{ ok: boolean; message?: string; data?: unknown }>;
  capabilityProfile: CapabilityProfile;
  skillStatus?: SkillStatusResult;
...
interface SmokeCheck {
  ...
  failMessage: string;
  remediation: string;
  validate?: (result: { ok: boolean; message?: string; data?: unknown }) => boolean;
  data?: unknown;
}

export async function runLiveSmoke(runner: LiveSmokeRunner): Promise<LiveSmokeResult> {
...
function buildLiveSmokeChecks(hasConfig: (key: string) => boolean): SmokeCheck[] {
  ...
  return [
    ...
    },
    {
      ...
      validate: result => Array.isArray(result.data) && result.data.some((item: any) => item?.status === 'Working')
    },
    {
