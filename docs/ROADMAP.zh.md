...
本文记录当前架构之外的后续演进方向。当前事实源是 [`ARCHITECTURE.zh.md`](./ARCHITECTURE.zh.md)。本文不是当前实现说明，也不是 Agent 必须立即执行的任务书。
...
## 2. Searcher factory registry 完整化
...
- README 不声称未来能力已经实现。
- Skill 不保存密钥、cookie、账号、密码或 token。
